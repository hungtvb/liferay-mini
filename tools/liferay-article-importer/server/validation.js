...
export function convertValue(value, target) {
  ...
  if (['double', 'float', 'number', 'decimal'].includes(dataType)) {
...
export function validateAndBuildPayload({mapping, rowNumbers, rows, structure, targets}) {
  const errors = [];
  const warnings = [];
  const payload = [];
  ...
  for (const target of targets) {
    ...
    if (target.required && !sourceHeader) {
      errors.push(issue({
        field: target.key,
        message: `Required target “${target.label}” is not mapped`,
        row: null
      }));
    }
    ...
    if (sourceHeader && !target.supported) {
      errors.push(issue({
        field: target.key,
        message: `“${target.label}” is nested or repeatable and is not supported by this importer version`,
        row: null
      }));
    }
  }
  ...
  rows.forEach((row, index) => {
    ...
    for (const target of targets) {
      ...
      try {
        converted = convertValue(rawValue, target);
      }
      catch (error) {
      ...
      if (converted == null) {
        if (target.required) {
          rowErrors.push(issue({
            field: targetKey,
            message: `Required value “${target.label}” is empty`,
            row: excelRow
          }));
        }
      ...
      if (targetKey === 'system.title') {
        article.title = converted;
      }
      else if (targetKey === 'system.externalReferenceCode') {
        article.externalReferenceCode = converted;
      }
    ...
    if (article.contentFields.length === 0) {
      warnings.push(issue({
        field: 'contentFields',
        message: 'No Structure field value will be sent for this row',
        row: excelRow,
        severity: 'warning'
      }));
    }
