...
for (const forbidden of ['LIFERAY_IMAGE_SOURCE_TYPE', 'LIFERAY_IMAGE_SOURCE_ID', 'LIFERAY_IMAGE_SOURCE_FOLDER_ID']) {
  if (content['server/config.js'].includes(forbidden) || content['.env.example'].includes(forbidden)) {
    throw new Error(`${forbidden} must be selected per import run, not required from ENV`);
  }
}
...
if (!content['server/liferay-client.js'].includes('/o/headless-asset-library/v1.0/asset-libraries')) throw new Error('Asset Library discovery is missing');
