...
function setStep(step, {scroll = true} = {}) {
  ...
  document.querySelectorAll('[data-step-target]').forEach((button) => {
    ...
    else if (target < step && target <= state.maxUnlockedStep) caption.textContent = 'Done';
    else if (target <= state.maxUnlockedStep) caption.textContent = 'Ready';
    else caption.textContent = 'Not started';
  });
...
function updateScopeSummary() {
  ...
  byId('summaryImageFolder').textContent = selectedValue('imageFolderSelect') ? selectedText('imageFolderSelect') : 'Source root';
...
function renderImageSourceTypes() {
  ...
  document.querySelectorAll('input[name="imageSourceType"]').forEach((input) => input.addEventListener('change', changeImageSourceType));
}
...
async function downloadTemplate() {
  ...
  try {
    const {data, response} = await api('/api/templates', {
      body: JSON.stringify({folderId: selectedValue('folderSelect'), structureId: selectedValue('structureSelect'), ...migrationScope()}),
      headers: {'Content-Type': 'application/json'},
      method: 'POST'
    });
    ...
    setStatus(byId('workbookStatus'), `${fileName} generated for the current scope.`, 'success');
...
function updateWorkbookFile() {
  ...
  byId('workbookFileMeta').textContent = file ? `${file.name} · ${formatFileSize(file.size)}` : `Maximum ${state.config?.maxUploadMb || '—'} MB`;
...
function renderValidation(validation) {
  ...
  byId('validationRows').innerHTML = rows.length ? rows.map((row) => `
    <tr><td>${escapeHtml(row.row)}</td><td><code>${escapeHtml(row.externalReferenceCode || '—')}</code></td><td>${escapeHtml(row.title || '—')}</td><td><span class="row-status ${row.status === 'VALID' ? 'valid' : 'blocked'}">${escapeHtml(row.status)}</span></td></tr>
  ...
  byId('issues').innerHTML = issues.map((item) => `
    <article class="issue ${item.severity === 'warning' ? 'warning' : ''}"><div><strong>${escapeHtml(item.code)}</strong><span>Row ${escapeHtml(item.row ?? '—')} · ${escapeHtml(item.field || 'workbook')}</span></div><p>${escapeHtml(item.message)}</p></article>
  `).join('');
...
function updateImportButton() {
  ...
  const strategyBlocked = (createStrategy === 'INSERT' && collisions) || (createStrategy === 'UPSERT' && !upsertConfirmed);
  ...
  button.disabled = !state.sessionId || !state.validation?.canImport || strategyBlocked || Boolean(state.taskId) || state.submissionLocked;
}
...
async function pollTask(taskId) {
  ...
  while (Date.now() - startedAt < state.config.pollTimeoutMs) {
    ...
    if (terminalStatuses.has(data.executeStatus)) {
      showToast(data.executeStatus === 'COMPLETED' ? 'Batch import completed' : `Batch import ended with ${data.executeStatus}`, data.executeStatus === 'COMPLETED' ? 'success' : 'error');
      return;
    }
...
async function startImport() {
  ...
  try {
    ...
    byId('taskHeading').textContent = error.code === 'BATCH_SUBMISSION_UNKNOWN' ? 'Submission requires manual verification' : 'Import failed';
